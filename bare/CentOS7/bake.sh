...
cat $tmpdir/etc/redhat-release | awk '{print $4}' > $outdir/.release

echo -ne "\nCreating CentOS Archive  ... "
tar --numeric-owner -cf - -C $tmpdir . | gzip > $outdir/centos7.tar.gz && echo "done" || echo "failed"

cd $outdir
stat -c "   %n: size=%s time='%x'" centos7.tar.gz
echo